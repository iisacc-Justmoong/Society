...
QString ModelMergeController::outputPathForName(const QString &name, const QString &directory, const QString &mode) const
{
    ...
    return QDir(parent).filePath(stem + (mode == "unified" ? ".iildmodel" : ".safetensors"));
}
...
bool ModelMergeController::run(const QVariantMap &options, bool validateOnly)
{
    ...
    if (base.isEmpty() || !QFileInfo(base).isFile()) return fail(tr("Choose an existing checkpoint file or a single-checkpoint .iildmodel package."));
    ...
    if (mode == "unified" ? suffix != "iildmodel" : (suffix != "safetensors" && suffix != "safetensor"))
        return fail(tr("Unified output requires .iildmodel; weighted arithmetic requires .safetensors."));
...
void ModelMergeController::finish(int exitCode, bool crashed)
{
    ...
    if (success) {
        ...
        if (m_validationOnly) {
            ...
            m_status = tr("Merged %1 tensors from %2 models. Base weight: %3. Original models were kept.")
