...
public:
    ...
    Q_INVOKABLE bool configureServer(const QUrl &url);
...
protected:
    // Native entry points select the build platform; integration fixtures can
    // construct both device kinds without making the role a mutable setting.
    NetworkDriveController(Mode deviceMode, DiscoveryService *service, QHostAddress bindAddress, QObject *parent);
private:
    QJsonObject handleGeneration(const QString &peer, const QJsonObject &request);
#if !defined(Q_OS_IOS) && !defined(Q_OS_ANDROID) && !defined(Q_OS_WASM)
    iiSocietyGeneration::Host m_generationHost;
#endif
    ...
    void invalidateHostConnection();
    QString accountSyncScope() const;
    QString m_registeredHost, m_registeredContainer, m_registeredScope;
    QString accountHost() const;
    QString accountContainer() const;
    bool pairingAuthenticated() const;
    ...
    QString m_validatedHost;
    QStringList m_syncHosts;
