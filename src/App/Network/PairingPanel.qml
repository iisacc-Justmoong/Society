...
LV.Sheet {
    ...
    scrollContent: false
    onOpened: {
        useQr = pairing.incomingName.length === 0
    ...
    Controls.ScrollView {
        ...
        ColumnLayout {
            ...
            LV.Label {
                ...
                text: panel.desktop ? qsTr("Sign in to the same account on both devices. Scan this QR to connect to your registered Society host.")
                    : qsTr("Sign in to the same account on both devices. On your registered desktop host, open Devices → Pair with QR code. Use the same Wi-Fi or LAN.")
                wrapMode: Text.Wrap
                sizeToContentHeight: true
            }
            LV.Label {
