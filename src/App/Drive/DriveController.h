...
class DriveController : public QObject
{
    Q_OBJECT
    QML_ELEMENT
    Q_PROPERTY(iisacc::accounts::AccountManager* accountManager READ accountManager WRITE setAccountManager NOTIFY accountManagerChanged)
    Q_PROPERTY(QString accountDriveStatus READ accountDriveStatus NOTIFY accountDriveStatusChanged)
...
public:
    ...
    iisacc::accounts::AccountManager* accountManager() const { return m_account; }
    void setAccountManager(iisacc::accounts::AccountManager* manager);
    QString accountDriveStatus() const { return m_accountDriveStatus; }
    Q_INVOKABLE bool saveContainerToAccount();
    Q_INVOKABLE bool useMovedContainer(const QUrl& image);
    bool hasDrive() const;
    ...
    Q_INVOKABLE bool createContainerAt(const QString &location);
    Q_INVOKABLE bool openContainerUrl(const QUrl &url);
    Q_INVOKABLE bool openContainerImage(const QUrl &url);
    Q_INVOKABLE QString localContainerPath(const QUrl &url) const;
...
signals:
    ...
    void accountDriveStatusChanged();
...
private:
    ...
    void prepareDisk(const QString &location = {}, bool create = false, const QString &expectedId = {}, bool publish = false);
    void applyAccountDrive();
    void setAccountDriveStatus(const QString& status);
    QPointer<iisacc::accounts::AccountManager> m_account;
    QString m_accountDriveStatus, m_accountDriveRevision, m_pendingAccountSubject;
    bool m_publishToAccount = false;
