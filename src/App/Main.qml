...
LV.ApplicationWindow {
    ...
    function showStorage(section) {
        if (drive.contentsAvailable) drive.openSection(section.length > 0 ? section : "files")
        else drive.goHome()
        selectedTab = "Storage"
    }
    ...
    nativeTitleBarHeight: onboardingRequired ? 56 : societyView.toolbarHeight
    windowDragHandleHeight: onboardingRequired ? 56 : societyView.toolbarHeight
    windowDragExclusionItems: onboardingRequired ? [] : societyView.toolbarInteractiveItems
    ...
    DriveController {
        ...
        accountManager: session.manager
    ...
    content: [
        OnboardingView {
            anchors.fill: parent
            anchors.topMargin: root.mobileLayout ? root.contentTopInset : 56
    ...
    DropArea {
        ...
        enabled: !root.onboardingRequired && !modelImporter.busy
