...
QJsonObject photoSnapshot(PhotoStore &store, bool ok) {
    ...
    return {{"ok", ok}, {"error", store.errorString()}, {"records", records}, {"previews", previews}};
}
}
PhotoController::PhotoController(Sender sender, QObject *parent, std::shared_ptr<PhotoLibrary> library)
    : QObject(parent), m_send(std::move(sender)), m_library(library ? library : nativePhotoLibrary()), m_customLibrary(bool(library)) {
...
void PhotoController::configure(const QString &container, bool enabled, const QStringList &authorized, const QStringList &hosts) {
    ...
    m_refreshing = true;
    work([](PhotoStore &store) { const bool ok = store.open(); return photoSnapshot(store, ok); }, [this](const auto &result) {
        m_refreshing = false;
...
void PhotoController::updateEntries(const QJsonArray &records) {
    ...
    for (const auto &value : records) {
        const auto record = value.toObject(); if (record.value("deleted").toBool()) continue;
        auto row = record.toVariantMap();
        row["preview"] = QUrl::fromLocalFile(m_container + "/Photos/.previews/" + record.value("id").toString() + ".jpg");
        row["previewStamp"] = m_previewStamps.value(record.value("id").toString()).toString();
        entries.append(row);
    }
