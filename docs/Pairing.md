...
같은 계정의 검증된 기기는 이제 기본적으로 자동 대기열에 등록되어 연결된다. 호스트 선택·재시도·계정 증명은 [자동 페어링](AutomaticPairing.md)을 따른다. 아래 수락·코드 비교와 QR 절차는 수동 연결 계약이다.

Dreamscapes는 새 링크를 직접 소비하는 별도 클라이언트로 연결할 수 있다. 데스크톱 QR 화면의 **Copy pairing link for Dreamscapes**를 사용하며 모델 기반 이미지 생성은 [호스트 생성](Generation.md)에 설명한다. 이 연결에서도 Files 공개 루트는 그대로 유지한다.
