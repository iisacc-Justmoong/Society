...
| iiSocietySync 0.8.0 | Namespace authority·리비전 그래프·변경 저널·저장 공급자·청크 복제·원격 Files 탐색 |
| iiSocietyContainer 0.14.0 | 드라이브 UUID·9개 영역·로컬 공유 및 Files 전용 OS 투영 |
| iiServerHost 0.6.0 | 계정으로 인증된 일반 연결과 TLS, 요청·응답·Files 전용 전송 원시 기능 |
...
자체 호스팅과 클라우드 호스팅은 동일한 authority 모델이다. 고정 호스트가 진실 소스이며, 논리 Namespace 아래의 Revision Graph → Change Journal → Object Metadata가 상태를 결정한다. Mac Storage·NAS·S3는 호스트 권한 아래의 저장 공급자이다. 제공자 변경과 연결 장애가 다른 primary host를 만들지 않는다. Mac 호스트는 연결된 peer가 없어도 로컬 변경을 확정하며, 클라이언트의 오프라인 편집은 제출 대기 상태로 남는다. 서버 relay와 LAN/TLS는 같은 저널을 복제한다.

SDK의 `objectMetadata()`는 확정 상태, `record()`는 작업 상태를 반환한다. 공급자 배치/복구는 `Controller::placeObject`/`restoreObject`와 `ObjectProvider`를 사용한다. 외부 공급자의 endpoint·자격 증명은 호스트 설정이며 다른 기기에 복제하지 않는다. 현재 배치는 명시적 파일 단위이며 앱의 로컬 투영을 유지한다. [SDK Namespace 계약과 검증 범위](../../../SDK/iiSocietySync/Namespace.md)를 따른다.

호스트의 로컬 저널은 캐시된 계정 세션의 안정적인 scope로 열린다. LAN 페어링 증명을 갱신할 수 없거나 증명이 만료돼도 로컬 변경 확정은 계속된다. 이 scope는 원격 접근 권한이 아니며 다른 기기와의 연결은 유효한 계정 증명·TLS 검사를 계속 요구한다. 로그아웃하거나 런타임을 중지하면 해당 계정의 저널 작업도 닫는다. `Society.Account`는 페어링 증명이 있는 경우와 없는 경우 모두 인터넷·peer 없이 확정·재열기를 검사한다.

## 실행 흐름

오프라인 호스트 회귀 검사는 대상 파일의 메타데이터가 스토리지 맵에 공개될 때까지 기다린 뒤 런타임을 중단한다. 게시된 리비전이 보존되고 최종 head와 파일 기록이 재열기 후에도 유지되는지 확인한다. 초기 폴더 등록도 head를 변경하며 화면 알림은 비동기이므로, 임의의 head 변경만으로 해당 파일의 저장이 끝났다고 판단하거나 화면 snapshot을 최종 head와 동일시하지 않는다.
...
4. Sync는 인증된 호스트의 descriptor를 확인하고 첫 연결에서 클라이언트의 기존 내용을 비공개 복구 영역에 보존한다. 호스트 UUID를 채택하고 전체 초기 상태 및 전송 중 변경을 내려받은 후에만 앱·OS에 미러를 공개하고 업로드를 허용한다. 이후 클라이언트는 기준 리비전과 변경을 제출하고 호스트가 확정한 리비전·저널·결과 파일을 받는다. 업로드 ACK 이후에도 확정 결과 수신까지 회차를 유지한다.
...
최초 클라이언트 내용은 `.society-sync/detached/<이전 UUID>-<마이그레이션 UUID>/`에 영역별로 보존하며 호스트에 섞지 않는다. SQL 이동 계획·원자적 UUID 채택·중단 복구로 영역 루트는 항상 유지한다. SHA-256 검증 후 파일 단위로 원자적 교체한다. 손상·중단 시 기존 목적지를 유지하고 다음 회차에 재시도한다. 동시 편집의 다른 내용은 충돌 사본으로 남긴다. 오래된 기준의 파일 제출은 호스트의 확정 상태를 유지하고 다른 파일 내용을 충돌 사본으로 보존한다. 비어 있지 않은 디렉터리를 삭제나 파일로 덮지 않는다. 대소문자·정규화 충돌은 오류로 드러낸다. 동일 호스트가 다른 드라이브를 선택하면 이전 미러·미전송 변경을 보존하고 새 드라이브를 초기 미러링한다. 다른 primary로의 임의 전환은 거부한다.
...
호스트의 파일명·논리 경로·크기·버전과 작은 프리뷰를 먼저 받는다. Files와 Generation History는 원본이 없어도 항목을 표시한다. 일반 파일 열기와 갤러리의 View original은 해당 파일의 다운로드가 끝나야 실행된다. Photos도 alias·프리뷰만 자동 수신하고 원본 열기/다운로드 시 원본을 받는다. 로컬에서 완성한 파일은 호스트로 업로드하며 전원이 꺼지거나 연결이 끊겨도 파일과 저널이 재시도의 근거로 남는다.

모바일 Society와 소비 앱은 `.society-sync/client-session.lock`으로 동일 기기의 세션을 직렬화한다. Society의 백그라운드 회차 종료나 실행 허가 만료 시 작업을 닫고 점유를 해제하며, Dreamscapes는 앱 내부 SDK로 이어받는다. 계정 scope는 AccountSession의 단일 계산을 공유한다. 다른 계정, 미검증 피어와 다른 primary 호스트로의 변경은 허용하지 않는다. 최초 로그인·호스트 선택은 Society에서 수행한다.
Files 화면은 선택 파일을 받는 동안 다운로드 상태를 표시한다. 호스트 버전 변경 등의 실패는 재선택 안내로 표시하고, 파일 공개가 끝나기 전 외부 뷰어를 열지 않는다. `Society.Gui`는 목록만 있는 파일의 대기·실패·재요청·완료 후 열기를 검사한다.

## 회귀 검사 환경

CTest는 로그인 셸에서 상속한 `DYLD_*` 라이브러리 경로를 비워 빌드가 연결한 SDK와 LVRS를 검증한다. 이전 설치본이 새 SDK나 QML 모듈을 덮어쓰지 않게 한다. 선택 다운로드 검사는 호스트 업로드 승인 완료 후의 캐시 무효화와 승인 전 로컬 변경 보존을 구분한다.
