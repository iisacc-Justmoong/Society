...
베이스와 재료를 선택하면 출력 이름을 입력하기 전에도 SDK 구조 검사가 자동 실행된다.
입력 변경은 650 ms 동안 모아서 처리하고 이전 요청은 취소한다. 응답은 요청 당시의
입력 revision과 현재 revision이 일치할 때만 표시한다. 실제 병합 중에는 자동 검사가
동시에 실행되지 않는다. 검사는 모델을 출력하지 않지만 기존 패키지·레거시 입력의
변환 캐시는 사용할 수 있다.

SDK 보고서를 단일 기준으로 계열, UNet·DiT·VAE·텍스트 인코더별 텐서 수,
병합 가능·조건부 가능·제외 상태, LoRA의 실제 대상 계층을 표시한다. 계열만 같은
재료를 곧바로 완전 호환이라고 표시하지 않는다. 전체 구조 보고서에서 대상 이름,
행 범위, 자료형, 형태와 제외 사유를 확인하고 복사할 수 있다. 구조 검사는 값 전체를
읽는 검사가 아니며, 실제 실행 시의 수치 보정과 구분한다.

예상 결과에는 포함·제외 재료 수, 형태를 맞추는 텐서 수와 위험 요인을 표시한다.
Weighted sum/difference는 베이스 텐서 좌표에서 계산한다. Unified는 독립 모델들의
순차 이미지 보정 패키지이지 하나의 신경망으로 변환하는 기능이 아니다.

출력 후 SDK가 저장된 모든 텐서를 다시 읽어 계산값과 이름·형태·자료형을 대조하고,
베이스 대비 실제 변경 수를 검증한다. Unified는 저장된 모든 모델 멤버의 해시를
검증한다. 앱 완료 문구에 재검증 범위를 표시하며, 재료가 전부 제외되거나 값 보정만
이루어진 경우에는 베이스 보존·베이스 보정 결과라고 명시한다. 이 검증은 출력 무결성
검증이며 생성 품질 보증이 아니다. `/Applications`가 아닌 `build/bin/Society.app`를
개발 빌드 산출물로 사용한다.

검증 완료 보고가 없는 구형·사용자 지정 실행기가 단순히 파일만 만든 경우에는 앱이
성공으로 표시하지 않는다. 이미 만들어진 파일은 보존하고 실행기 갱신과 확인이
필요하다는 메시지를 표시한다.
...
각 추가 리소스 카드에는 항상 독립적인 실수 가중치 입력을 표시한다. 입력은 유한한 0 이상의 값이며 `0.25`, `1.5`, `1e-3`처럼 소수와 지수 표기를 받을 수 있다. `0`은 리소스를 목록과 구조 검사에는 남기되 계산 기여를 없앤다. 행을 추가·삭제해도 가중치는 해당 행의 모델과 함께 이동하며 실행기에는 화면 순서 그대로 `--weights` 배열을 전달한다. Weighted sum의 추가 체크포인트 합이 1을 넘으면 비율을 유지해 정규화하고 남은 지분을 베이스가 가지며, Weighted difference의 베이스 계수는 1이다. Unified 체크포인트 강도는 0~1, LoRA 강도는 0 이상의 실수이다. 캐시와 실행기·Python 경로는 **Advanced settings**를 펼쳐 편집하며 접어도 값을 보존한다. 입력 변경 시 이전 검사를 현재 입력의 검사 성공으로 표시하지 않는다. 실행 중에는 입력·출력 설정을 잠그고 검사 영역에서 취소·경과 시간·결과 폴더·보고서를 제공한다.
...
**Inspect inputs (optional)**는 SDK `--inspect`로 전체 가중치 해시 전에 베이스 생태계와 재료별 포함·제외 계획을 보고하며 병합의 선행 조건이 아니다. 모델 선택 시 각 행 아래 Description에 감지한 생태계와 베이스 호환 여부를 즉시 표시한다. FLUX.2/Krea 2, SDXL/Illustrious/Pony처럼 같은 생태계이거나 미분류 텐서 레이아웃이 정확히 일치하는 체크포인트만 포함하며, LoRA는 베이스에 모든 대상이 실제로 매칭될 때만 포함한다. 호환되지 않는 재료 하나 때문에 전체 병합을 중단하지 않고 그 재료만 제외한다. 20개 중 11개가 호환되면 11개의 가중치만 남기고 합 방식의 베이스 계수도 다시 계산한다. 가중 합·차에서 포함 가능한 재료가 하나도 없으면 베이스만으로 출력하고 실질적인 재료 기여가 없음을 표시한다. Unified는 포함 가능한 재료가 필요하다. Society는 `--checkpoint-policy common-layer`와 `--lora-policy strict`을 전달한다. 같은 생태계의 체크포인트는 베이스 텐서 좌표계를 출력 계약으로 삼고, 정확히 일치하는 주소를 우선한 다음 역할·깊이·종류·형상이 가까운 텐서를 축별 좌표 재표본화하여 끼워 맞춘다. `denoiser.sigmas`처럼 재료에 없는 스케줄 좌표와 비부동소수점 상태는 무관한 가중치를 대입하지 않고 베이스 값을 유지한다. 다른 생태계나 매칭되지 않는 LoRA는 합성 투영하지 않는다. **Copy full report**는 `resource_compatibility`와 `excluded_sources`를 포함한다. SDK 실행 파일 선택은 환경 변수, 앱 옆 실행기, CMake 지정 실행기, 사용자 설치 경로, PATH 순서이다.

SDK 투영 정책 `base-layout-normalize-project-flatten-v3`은 이름 정규화와 상대 블록 깊이 대응 후 전치·동일 원소 수 재배열·축별 재표본화·차원 차이의 평탄화 재표본화를 순서대로 사용한다. 스케일/영점이 있는 INT8·UINT8·FP8은 복원한 값을 계산하고 베이스의 양자화 형식으로 되돌린다. 잘못된 양자화 인자는 해당 텐서만 보존하고 이유를 보고한다. 누락 재료는 합에서 그 몫을 베이스에 반환하고 차에서는 0으로 처리하여 베이스 자체가 차감되는 문제를 방지한다. 스케줄·상태·스케일·빈 텐서는 보존하되 부동소수점 비정상 값은 아래 규칙으로 보정하며, 다른 구성요소나 패키지 단계의 가중치를 대신 쓰지 않는다. **Copy full report**의 `common_layers.transform_counts`, `runtime_events`, `numeric_normalization`에서 적용 경로와 보존 이유를 확인할 수 있다. 결과는 실험적 좌표 투영이며 학습된 모델 변환이나 생성 품질을 보증하지 않는다.

추가 체크포인트 가중치의 NaN·양의 무한대·음의 무한대는 병합 전체를 실패시키지 않는다. 양자화 복원·형상 보정 후 비정상 좌표만 합에서는 보정된 베이스 값으로, 차에서는 0으로 대체한다. 정상 좌표와 다른 재료의 기여는 그대로 계산한다. 텐서 전체가 비정상이고 다른 기여가 없으면 베이스를 복사하고 병합 수에 넣지 않는다. **Copy full report**의 `numeric_normalization.nonfinite_material_values`, `nonfinite_material_tensors`, `nonfinite_material_examples`에 보정 수와 재료 인덱스, 텐서명, 최대 8개의 베이스 좌표 예시를 기록한다.

가중 합·차의 `repair_policy: best-effort-v1`은 베이스와 LoRA 인자의 NaN·무한대를 0으로 대체한다. LoRA 계산 후에도 비정상인 델타는 0으로, 계산 자체가 불가능한 델타는 생략으로 처리한다. 출력 NaN은 보정된 베이스 좌표로 대체하고 무한대·표현 범위 초과는 FP8·FP16 등 베이스 자료형의 유한한 최댓값·최솟값으로 제한한다. 빈 파일·없는 경로·손상된 **추가 재료**는 제외하며, 나머지 정상 재료의 기여는 유지한다. 베이스만 남으면 `result_kind: base-fallback`과 `no_effect: true`, 베이스의 비정상 값만 고쳤으면 `repaired-base`로 명시한다. 앱도 이를 일반 병합 완료와 구분한다. 보정 수와 최대 64건의 `repair_events`는 전체 보고서에 남긴다. 엄격 정책은 비정상 값과 오버플로를 계속 거부한다. Unified에서 그대로 복사하는 멤버 파일까지 수치 보정하는 기능은 아니다.

읽을 수 없는 베이스·비어 있는 베이스 텐서 목록, 실행 중 원본 변경, 기존 출력 충돌, 저장 실패, 실행 환경 부재와 메모리 부족은 여전히 실패로 처리한다. 자동 보정은 원래 학습된 값을 복원하지 않으며 원본 파일에 직접 적용하지 않는다.

엄격 정책의 Anima `model.diffusion_model.*`·`net.*` 대응은 전체 목록의 일대일 대응과 형상·자료형·예측 설정 검사를 유지한다. 모든 정책은 베이스의 이름을 출력에 보존하며 원본 파일을 변환하거나 이름을 바꾸지 않는다.
...
베이스는 단일 전체 체크포인트 파일 또는 `.iildmodel` 패키지이고, 첫 추가 모델은 필수이다. 추가 모델은 단일 체크포인트, `.iildmodel`, LoRA 파일·어댑터 폴더이며 **Add material**로 개수 제한 없이 늘리고 **Remove**로 제거한다. **입력 모델은 현재 컨테이너의 `Models/` 목록에서 선택한다.** LVRS `ListItem` 모델 행을 누르거나 우클릭하면 `ContextMenu`가 열리며, 파일 경로를 입력하거나 운영체제 파일·폴더 선택기를 열 필요가 없다. Society의 시스템 파일 공급자가 `Files/`만 노출하더라도 Models의 모델을 선택할 수 있다.

`MergeModelCatalog`는 Qt Concurrent에서 iiSocietyContainer의 `ModelStore`로 Models 하위 폴더를 비동기 조회한다. `.safetensors`, `.safetensor`, `.ckpt`, `.pt`, `.pth`, `.bin`, `.iildmodel` 파일을 표시하며, `adapter_config.json`이 있는 어댑터 폴더는 내부 가중치를 중복 나열하지 않고 하나의 항목으로 표시한다. 일반 Diffusers 폴더와 그 내부 구성원은 단일 출력 체크포인트를 보장하기 위해 병합 선택 목록에서 제외한다. 단일 파일 패키지와 과거 디렉터리형 패키지를 포함한 유효한 `.iildmodel`은 stage 수나 내장 LoRA 유무와 무관하게 하나의 선택 항목으로 표시한다. 유형·저장 형식을 함께 표시하며, 어댑터 폴더와 LoRA/LyCORIS/DoRA로 분류된 파일은 추가 재료 목록에만 표시한다. 설정 파일 없는 단일 LoRA도 헤더에서 판정할 수 있다. 빈 파일, 숨김·스테이징 항목, 심볼릭 링크와 지원하지 않는 확장자는 목록에서 제외한다. 파일의 실제 체크포인트/LoRA 종류와 모델 호환성은 SDK가 병합 시 판별하며 목록 조회는 텐서를 로드하지 않는다.
...
자동 합은 베이스와 추가 체크포인트에 동등한 비중을 주고, 자동 차는 추가 체크포인트마다 `0.5`를 뺀다. LoRA의 자동 강도는 두 방식 모두 `1`이며 베이스의 체크포인트 비중을 차감하지 않는다. 합에서 포함된 추가 체크포인트 계수의 합이 1을 넘으면 비율을 유지하며 합계 1로 정규화하고 베이스 계수는 0으로 둔다. LoRA 강도와 차 방식 계수는 이 정규화의 대상이 아니다. `weight_normalization`에 요청값·적용값을 함께 기록한다.
...
**Inspect inputs (optional)**는 생태계와 포함·제외 계획을 미리 보여 줄 뿐이며 실행 전에 눌러야 할 필요가 없다. **Merge models**는 즉시 병합을 시작하고, 베이스와 호환되는 체크포인트와 LoRA만 계산하며 나머지는 보고서에 제외 사유를 남긴다. 레거시 포맷은 변환 캐시를 사용하며, 실제 산술에서 비유한 값·오버플로·출처 해시를 확인한다. 베이스 텐서의 이름·shape·dtype을 보존하므로 정상적으로 병합해도 용량은 같거나 거의 같을 수 있다. 성공 판정은 용량이 아니라 `changed_tensor_count`와 출력 SHA-256을 사용한다. 다중-stage `.iildmodel` 베이스는 결과도 `.iildmodel` 패키지로 저장하고 변경된 stage의 해시·크기를 manifest에 반영한다. 성공하면 저장 위치와 SDK 보고서를 제공하며 **Open output folder**로 결과 부모를 연다. **Copy full report**는 출처·실제 강도·호환성·제외 사유·출력 해시를 포함한 전체 JSON을 복사한다. 화면 보고서는 처음 16,000자만 표시한다.
...
- `automaticRecoveryUsesInstalledRuntime`의 실제 설치 SDK 호출: 비정상 베이스·FP16 오버플로·누락 및 손상 재료의 동시 보정, 계수 정규화, 베이스만 출력·베이스만 보정한 출력의 정확한 수치·화면 상태·덮어쓰기 방지.
- 단일 Safetensors 출력 강제, 이름·출력 경로 누락 거부, Diffusers 베이스와 통합 모드 거부, 베이스 생태계 기준의 부분 포함·제외와 가중치 재계산, 같은 생태계의 공통 레이어 매핑, 손상 입력의 오류·무출력, 원본·기존 결과 덮어쓰기 차단, 취소 후 원본 보존.
