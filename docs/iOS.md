...
인증된 준비 상태에서 `sync_store_busy`, `file_changed_during_scan` 같은 콘텐츠 동기화 오류는 상태에 표시하고 재시도하며 온보딩으로 되돌리지 않는다. 호스트·계정·컨테이너 또는 프로토콜 불일치와 연결 종료는 계속 준비 상태를 해제한다. 준비 상태는 콘텐츠 큐의 성공 여부가 아닌 현재 연결에서 확인한 호스트 검증으로 결정한다.
